...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to put me against oh-my sexy Keyaru-Sama huh?")
        elif user_id in DEV_USERS:
            message.reply_text("I can't act against our Healers.")
        elif user_id in DRAGONS:
            message.reply_text(
                "Fighting this Knight here will put normies lives at risk."
            )
        elif user_id in DEMONS:
            message.reply_text(
                " Damnnn Its a Magic Hero I can't ban him."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from the upper level Heroes to ban this Rifle Hero ."
            )
